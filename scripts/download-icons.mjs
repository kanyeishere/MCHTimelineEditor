...
const APP_SOURCE = readFileSync('app.js', 'utf8');
const actionsSource = APP_SOURCE.match(/const actions = \[([\s\S]*?)\]\s*\.map/)?.[1];

if (!actionsSource) {
  throw new Error('Unable to find actions array in app.js');
}
...
for (const line of actionsSource.split('\n')) {
  ...
  if (!id || !en) throw new Error(`Unable to parse action line: ${line}`);
  actions.push({ id, en });
}
...
for (const action of actions) {
  ...
  const destination = `assets/icons/${action.id}.png`;
